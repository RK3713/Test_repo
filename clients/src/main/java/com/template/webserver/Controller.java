...
//    }
    ...
    public List<StateAndRef<TitleState>> getIOUs() {
        // Filter by state type: IOU.
        return proxy.vaultQuery(TitleState.class).getStates();
    }
    ...
    public ResponseEntity<String> issueIOU(HttpServletRequest request) throws IllegalArgumentException {
        ...
        try {
            ...
            SignedTransaction result = proxy.startTrackedFlowDynamic(TemplateFlow.Initiator.class, amount,otherParty).getReturnValue().get();
            // Return the response.
    ...
    public ResponseEntity<List<StateAndRef<TitleState>>> getMyIOUs() {
        List<StateAndRef<TitleState>> myious = proxy.vaultQuery(TitleState.class).getStates().stream().filter(
